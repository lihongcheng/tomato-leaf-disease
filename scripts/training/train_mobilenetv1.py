...
def main():
    ...
    try:
        ...
        print(f"从timm加载预训练MobileNetV1失败，尝试不使用预训练权重: {e}")
        model = timm.create_model('mobilenetv1_100', pretrained=False)

    in_features = model.get_classifier().in_features
    model.classifier = nn.Linear(in_features, len(train_set.class_map))
